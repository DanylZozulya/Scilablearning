...
n=-1
disp("")
while (n<2) | (n>10^6)   //введення змінної n до кінця циклу//
  n=input("введите n")
end
...
for i=1:1:999
    x=primes(n(i))
    n2=size(x)   //пошук всіх простих чисел і розрахунок їхньої кількості//
    y1=[y1,n2(2)]
end
y2=n./log1p(n)
plot(n,y1,'red')
...
disp("start <circle>")     
for i=1:1:N2
    while x(i)<10^4            //якщо елемент має менше ніж 10 цифр - домножаємо його на 10,доки воно не буде 10-цифровим//
        x(i)=x(i)*10           
    end
    ...
    for k=n:1:n1(2)
        if x(i)==p(k)          ///перевірка чи належить елемент матриці простим числам,якщо належить - к-ть+1 і перехід до наступного елемента//
            kilkist=kilkist+1  
            break
        end
    end
